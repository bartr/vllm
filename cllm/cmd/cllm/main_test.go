...
func TestRunHelp(t *testing.T) {
	...
	for _, want := range []string{"Usage: cllm [options]", "--downstream-url", "--downstream-token", "--downstream-model", "--models-cache-ttl", "--version", "-h, --help", `Default system prompt for /ask (default "You are a helpful assistant.")`, "Default max tokens for /ask (default 4000)", "Default temperature for /ask (default 0.2)"} {
