...
1. **KV cache and memory pressure modeling — partially shipped (Phase 1+2 of [design-memory-pressure.md](cllm/docs/design-memory-pressure.md)).** Each node now carries an optional `max_kv_tokens` budget separate from its compute budget, and admission gates on both axes (`kv_pressure` / `kv_oversize` rejection reasons join `over_capacity`). The `f(load)` curve consumes `combined_load = max(cost_load, kv_load × kv_weight)` so memory-bound regimes degrade independently of compute fill. New `:dsl kv-cost=N` / `:dsl no-kv` directives, three new profiles (`kv-light`, `kv-heavy`, `kv-stress`), and four `cllm-overview` panels (KV occupancy, combined load, rejection reasons, cost-vs-KV crossover) close the most prominent §6.6 caveat. Remaining work: per-class KV inheritance shorthand in `nodes.yaml`, KV-aware completion estimator (currently `kv_cost = total_cost`), and preemption-on-pressure are all left as Phase 4+ extensions.
...
14. **Workload class as a dimension orthogonal to tenant *and* to node class** — today, "interactive" and "batch" live as DSL profiles (§12.5), and `nodes.yaml` already defines a *node* class (`H100`, `A10`, …) used by the router. Promoting workload behavior to a first-class `workload_class` dimension separates three things that are currently entangled: *who* (tenant: customer / team / workload owner), *what hardware* (node class), and *what kind of work* (workload class: `interactive`, `batch`, `eval`, `background`). Per-tenant token buckets continue to enforce quota and isolation (§6.5); node class continues to drive routing and capacity (§6.7); workload class controls priority, queue policy, and phase-aware allocation (§14, item 13). The `tenant × workload-class` matrix unlocks the canonical platform demonstration: *"customer-b runs a batch flood, but customer-a's interactive TTFT stays protected."* Sketch:
    ...
    Class is surfaced as a Prometheus label on every admission, lifecycle, and stream metric so dashboards partition by `tenant`, by `class`, and by their cross-product without separate runs (§8.2).
